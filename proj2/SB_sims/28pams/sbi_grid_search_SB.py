...
# Add this right before the prior initialization
def clear_gpu_memory():
    gc.collect()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
        print(f"GPU memory allocated before prior initialization: {torch.cuda.memory_allocated()/1e9:.2f} GB")
        print(f"GPU memory reserved: {torch.cuda.memory_reserved()/1e9:.2f} GB")

# Add this right before your SB28 prior initialization
clear_gpu_memory()

# Temporarily use CPU for prior initialization if needed
try:
    ...
    if "CUDA out of memory" in str(e):
        print("CUDA out of memory, trying CPU initialization instead")
        prior = initialise_priors_SB28(
            df=df_info, 
        ...
        # Move to GPU after if needed and possible
        if device == "cuda":
            try:
                prior = prior.to(device)
                print("Successfully moved prior to GPU after CPU initialization")
            except:
                print("Keeping prior on CPU")
    else:
        raise  # Re-raise if it's not a CUDA memory error
